package com.github.oauth.core.web;
...
import com.github.oauth.core.service.AccountService;
import com.github.oauth.core.service.AuthService;
import com.github.oauth.core.service.ClientService;
...
public class OAuthController
{
	...
	public Object authorize(HttpServletRequest request, HttpServletResponse response) throws OAuthSystemException, URISyntaxException
	{
		try
		{
			...
			Account account = (Account)request.getSession(true).getAttribute(SSOConstants.SESSION_USER_LOGIN);
			if(account == null)
			{
			...
			account = (Account)request.getSession(true).getAttribute(SSOConstants.SESSION_USER_LOGIN);
			//验证client id
			if(!clientService.checkClient(clientId))
			{
				throw new AuthorizeException("Invalid client id");
			}
			//设置code并跳转
			...
			if(ResponseType.CODE.toString().equals(responseType))
			{
				...
				authService.addCode(code, account.getOpenId());
			}
	...
	public void checkLogin(HttpServletRequest request)
	{
		String login = request.getParameter("login");
		String password = request.getParameter("password");
		Account po = accountService.login(login, password);
		request.getSession(true).setAttribute(SSOConstants.SESSION_USER_LOGIN, po);
		logger.info(String.format("Login Success,Account Login=%s", login));
	}
